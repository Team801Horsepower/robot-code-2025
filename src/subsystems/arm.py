...
class Arm(Subsystem):
    ...
    def periodic(self):
        if isinstance(self.target, Transform2d):
            ...
            max_point = (max(claw_bounds[0].x, claw_bounds[1].x), max(claw_bounds[0].y, claw_bounds[1].y))
            min_point = (min(claw_bounds[0].x, claw_bounds[1].x), min(claw_bounds[0].y, claw_bounds[1].y))
            if max_point.x + pivot_offset.x > (robot_dimensions.x/2 + bumper_distance + ik_boundary_distance):
                p_1.x -= max_point.x + pivot_offset.x - robot_dimensions.x/2 - bumper_distance ik_boundary_distance
                self._target_outofbounds = True
            elif min_point.x + pivot_offset.x < (-robot_dimensions.x/2 - bumper_distance - ik_boundary_distance):
                p_1.x += min_point.x + pivot_offset.x + robot_dimensions.x/2 + bumper_distance + ik_boundary_distance
                self._target_outofbounds = True
    ...
    def target_attainable(self) -> bool:
        return (self._target_outofbounds and self.pivot.target_attainable() and self.elevator.target_attainable() and self.wrist.target_attainable()
